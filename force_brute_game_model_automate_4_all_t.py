...
def bf_balanced_player_game_ONE_ALGO_OLD(arr_pl_M_T_vars_init, algo_name,
                            pi_hp_plus=0.02, 
                            pi_hp_minus=0.33,
                            a=1, b=1,
                            gamma_version=1,
    ...
    for t in range(0, t_periods):
        ...
        if manual_debug:
            ...
            phi_hp_plus_t = fct_aux.compute_benefit_energy_sold_by_SG_2_HP(
                                ...
                                a=a)
            pi_hp_minus_t = round(phi_hp_minus_t/q_t_minus, fct_aux.N_DECIMALS)
            pi_hp_plus_t = round(phi_hp_plus_t/q_t_minus, fct_aux.N_DECIMALS)
            pi_0_plus_t = round(pi_sg_minus_t_minus_1*pi_hp_plus_t/pi_hp_minus_t, 
                                fct_aux.N_DECIMALS)
            pi_0_minus_t = pi_sg_minus_t_minus_1
            if t == 0:
...
def test_BRUTE_FORCE_balanced_player_game_Pi_Ci_one_period_ONE_ALGO():
    ...
    setA_m_players = 5; setC_m_players = 5;                                    # 10 players
    m_players = setA_m_players + setC_m_players
    
    scenario_name = "scenario0"
    dico_scenario = {scenario_name: scenario0}
    arr_pl_M_T_vars_init = fct_aux.get_or_create_instance_Pi_Ci_etat_AUTOMATE_SETAC(
